...
def attempt_load(weights, map_location=None, cfg=None, ch=None,model1=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        if isinstance(ckpt,dict):
            ...
            model_com = model1.Combination(cfg, ch, nc, anchors=hyp.get('anchors'))
            model_com.load_state_dict(state_dict)
            m = model_com.float()
            m.Yolo_net.fuse()
            m.eval()
            model.append(m)
