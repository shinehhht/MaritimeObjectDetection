...
class Simam(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"simam image is {(x*self.activaton(y)).shape}")
        return x * self.activaton(y)

class TransformerLayer(nn.Module):
    # Transformer layer https://arxiv.org/abs/2010.11929 (LayerNorm layers removed for better performance)
    def __init__(self, c, num_heads, drop_path=0.):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.drop_path(self.fc2(self.fc1(x))) + x
        return x


class TransformerBlock(nn.Module):
    # Vision Transformer https://arxiv.org/abs/2010.11929
    def __init__(self, c1, c2, num_heads, num_layers,drop_path):
        super().__init__()
        self.conv = None
        if c1 != c2:
            self.conv = Conv(c1, c2)
        self.linear = nn.Linear(c2, c2)  # learnable position embedding
        self.tr = nn.Sequential(*[TransformerLayer(c2, num_heads, drop_path) for _ in range(num_layers)])
        self.c2 = c2
    def forward(self, x):
...
class Add2(nn.Module):
    ...
    def forward(self, x):
        if self.index == 0:
            print(f"origin x {x[0].shape}")
            print(f"forward x {x[1][0].shape}")
...
class myTransformerBlock(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop,drop_path):
        """
        :param d_model: Output dimensionality of the model
        :param d_k: Dimensionality of queries and keys
        :param d_v: Dimensionality of values
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.sa(self.ln_input(x)))
        x = x + self.drop_path(self.mlp(self.ln_output(x)))
...
class GPT(nn.Module):
    ...
    def __init__(self, d_model,drop_path=0.,h=8, block_exp=4,
        ...
        # transformer
        self.trans_blocks = nn.Sequential(*[myTransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop,drop_path[layer])
    ...
    def forward(self, x):
        ...
        #print(f"GPT fea shape {rgb_fea_out.shape}")
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"CONV shape {self.act(self.bn(self.conv(x))).shape}")
...
class SPPCSPC(nn.Module):
    ...
    def forward(self, x):
        # print("hello this is sppcspc")
        x1 = self.cv4(self.cv3(self.cv1(x)))
        y1 = self.cv6(self.cv5(torch.cat([x1] + [m(x1) for m in self.m], 1)))
        y2 = self.cv2(x)
        # print(f"SPP shape {self.cv7(torch.cat((y1, y2), dim=1)).shape}")
