...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        # print(f"x in attention shape {x.shape}")
        B_, N, C = x.shape
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        # print(f"basic transformer x shape {x.shape}")
        H, W = x_size
        B, L, C = x.shape
...
class SwinFusion(nn.Module):
    ...
    def __init__(self, img_size, embed_dim, patch_size=1,
                Fusion_depths=[2, 2],
                 Fusion_num_heads=[8, 8, 8],
    ...
    def forward(self, input):
        ...
        x = x.view(-1, self.embed_dim, self.img_size, self.img_size)
        y = y.view(-1, self.embed_dim, self.img_size, self.img_size)
...
if __name__ == '__main__':
    model = SwinFusion(img_size=80, embed_dim = 512)
    # print(model)

    x = torch.randn((6, 512, 80, 80))
    x = model([x,x])
