...
class Combination(nn.Module):
    def __init__(self, cfg, weights, ch=3, nc=None, anchors=None,name = None):
        super().__init__()
        self.Yolo_net = yolo.Model(weights,ch, nc, anchors)
    ...
    def forward(self, input_data, isp_flag,defog_A, IcA, batch_id=None, epoch=None, augment=False, profile=False):
        ...
        if isp_flag:
            with torch.amp.autocast(device_type='cuda',dtype=torch.float32):
                ...
                # print(f"resized data type is {input_data[0].dtype}")
        ...
        pred = self.Yolo_net(input_data,augment,profile)
