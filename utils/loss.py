...
class ComputeKLloss(nn.Module):
    def __init__(self,temp,weights):
        super(ComputeKLloss, self).__init__()
        self.temperature = temp
        self.scale_weights = weights
        
    def forward(self, clear_features, adapted_features):
        total_loss = 0
        
        for clear_feature, adapted_feature,weight in zip(clear_features, adapted_features,self.scale_weights):
            # print(f"compute KLloss, {clear_feature.shape}")
            B,C,H,W = clear_feature.shape
            
            clear_flat = clear_feature.view(B,C,-1)
            adapted_flat = adapted_feature.view(B,C,-1)
            
            softmax_clear = F.softmax(clear_flat / self.temperature, dim=-1) + 1e-10
            softmax_adapted = F.softmax(adapted_flat/ self.temperature, dim=-1) + 1e-10
            
            KL_div = softmax_clear * (torch.log(softmax_clear) - torch.log(softmax_adapted)) # [B,C,hw]
            KL_div = (KL_div.sum(dim=-1)).mean(dim=0) # [B, C] -> [C]
            # print(KL_div.shape)
            scale_loss = KL_div.sum() * (self.temperature ** 2)
            # print(scale_loss.shape)
            total_loss += scale_loss * weight
        # print(total_loss)
        return total_loss
...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7 为不同尺度的检测层分配权重，小目标层loss权重更大
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance = BCEcls, BCEobj, model.gr, h, autobalance
    ...
    def find_3_positive(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets       targets shape e.g (63,6),63 is targets num
        indices, anch = [], [] # indices store the assigned grid cell indices for each target.
        gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)     shape (na,nt)
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices      targetsRepeat (na, nt, 6). ai[:,:,None] (na,nt,1)
        # print(f"targets shape {targets.shape}") (3,63,7)
        g = 0.5  # bias
        off = torch.tensor([[0, 0],
                            [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m
                            # [1, 1], [1, -1], [-1, 1], [-1, -1],  # jk,jm,lk,lm
        ...
        for i in range(self.nl): # detect layer
            anchors = self.anchors[i]
            gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain    p[i] shape (batch, anchor, grid_x, grid_y, xywh+cls)

            # Match targets to anchors
            t = targets * gain # 乘缩放因子将target位置放到对应grid图里
            if nt:
                # Matches
                r = t[:, :, 4:6] / anchors[:, None]  # wh ratio
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare      找到那些尺寸差异不超过 anchor_t 倍的 anchor，认为它们可以负责预测这个目标框
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter
                ...
                gxy = t[:, 2:4]  # grid xy
                gxi = gain[[2, 3]] - gxy  # inverse
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T # (N,2).T = (2,N),一行x，一行y
                l, m = ((gxi % 1. < g) & (gxi > 1.)).T
                j = torch.stack((torch.ones_like(j), j, k, l, m))
                t = t.repeat((5, 1, 1))[j]
...
if __name__ == '__main__':
    clear_features = [torch.randn(6,32,20,20),torch.randn(6,32,20,20),torch.randn(6,32,20,20)]
    adapted_features = [torch.randn(6,32,20,20),torch.randn(6,32,20,20),torch.randn(6,32,20,20)]
    loss = ComputeKLloss(1)
    loss = loss(clear_features, adapted_features)